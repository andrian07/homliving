...
<script type="text/javascript">
    ...
    $.ajax({
      ...
      success : function(data){
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          '<a href="#" class="'+data[i].module_name+'" id="'+data[i].module_name+'change_permision" onclick="change_permision('+data[i].module_name+')" style="display:none;">Ubah</a></td>'+
          '</tr>'
        }
  ...
  function change_permision(id){
    ...
    $.ajax({
      ...
      dataType: "json",
      data: {permision:permision, view_value:view_value, add_value:add_value, edit_value:edit_value, delete_value:delete_value},
      success : function(data){
        if (data.code == "200"){
         document.getElementById("temp").innerHTML = text_temp; 
        } else {
